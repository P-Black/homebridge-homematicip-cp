...
exports.HmIPDoorLockDrive = void 0;
 ...
 *
 */
class HmIPDoorLockDrive extends HmIPGenericDevice_1.HmIPGenericDevice {
    service;
    ...
    constructor(platform, accessory) {
        ...
        this.openLatchFallback = this.accessoryConfig?.['openLatchFallback'] !== false;
        this.authorizationPin = (this.accessoryConfig?.['authorizationPin']
            ?? this.accessoryConfig?.['pin']
            ?? this.platform.authorizationPin
            ?? '');
    ...
    async handleLockTargetStateSet(value) {
        ...
        const targetLockState = this.getHmIPTargetLockState(value);
        const body = {
            channelIndex: 1,
            deviceId: this.accessory.context.device.id,
            authorizationPin: this.authorizationPin,
            targetLockState: targetLockState,
        };
        const result = await this.platform.connector.apiCall('device/control/setLockState', body);
        if (result === false
            && value === this.platform.Characteristic.LockTargetState.UNSECURED
            && targetLockState === LockState.OPEN
            && this.openLatchFallback) {
            this.platform.log.warn('Door lock drive %s rejected OPEN. Retrying with UNLOCKED because openLatchFallback is enabled.', this.accessory.displayName);
            const fallbackBody = {
                ...body,
                targetLockState: LockState.UNLOCKED,
            };
            await this.platform.connector.apiCall('device/control/setLockState', fallbackBody);
        }
    }
...
exports.HmIPDoorLockDrive = HmIPDoorLockDrive;
